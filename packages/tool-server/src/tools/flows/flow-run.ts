...
/**
 * Yield every parsed step, recursing into `when:` blocks (the parser's only
 * nesting), each with the authored position that names it - the only handle a
...
function* walkSteps(steps: FlowStep[], within = ""): Generator<{ step: FlowStep; where: string }> {
  for (const [i, step] of steps.entries()) {
    ...
    yield { step, where };
    if (step.kind === "when") yield* walkSteps(step.steps, ` of the when: block at ${where}`);
  }
}
