 ...
 *
 * `device` is nullable here, unlike on {@link ActionEnv}: a flow that acts on no
 * device runs without one, and its failures (a cyclic `run:`, an unloadable
 * fragment) still deserve a failure block. Every device-backed section degrades
 * rather than assuming one — see the `no-device` screen state.
 */
