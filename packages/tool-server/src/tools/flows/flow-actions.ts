...
export interface ActionEnv {
   ...
   * to skip work it has been shown cannot pay off: a settle, and a read whose
   * failure the caller degrades past anyway. The skip is not silent: the
   ...
   * by the one place that can prove a source dead (a {@link settleTree} that
   * failed every read attempt) and cleared by {@link readFlowTree}, which every
