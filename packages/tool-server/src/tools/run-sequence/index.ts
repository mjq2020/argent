...
Allowed tools and their args (udid is auto-injected, do NOT include it in args):
  ...
  keyboard:       { text?: string, key?: string, clear?: boolean, delayMs?: number }  (clear → text → key; TV: text only)  [ios/android/chromium/vega/tv]
...
Example — type text then press Enter (one step: the key is pressed after the text is typed):
  { "udid": "<UDID>", "steps": [
