 ...
 * until the RPC timeout or describe UI nobody sees. Whether a deliberately
 * foregrounded system app's read would succeed is unverified, so the
 * conservative refusal stands until someone needs that case - and it keeps the
 * terminal signal that spares the native-* tools an unbounded restart-app →
 * retry loop.
 *
